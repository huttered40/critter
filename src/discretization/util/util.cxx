...
double get_arithmetic_mean(std::pair<bool,int> index){
  ...
  auto& pattern_list = index.first == true ? active_patterns : steady_state_patterns;
  return pattern_list[index.second].M1;
}
double get_variance(std::pair<bool,int> index){
  // returns variance
  auto& pattern_list = index.first == true ? active_patterns : steady_state_patterns;
  size_t n = pattern_list[index.second].num_schedules;
  if (n<=1) return 1000.;
  return pattern_list[index.second].M2 / (n-1.);
}
...
double get_confidence_interval(std::pair<bool,int> index, double level){
...
void error_test(std::pair<bool,int> index){
  ...
  bool decision = ((get_confidence_interval(index) / (2.*get_arithmetic_mean(index))) < pattern_error_limit) &&
                  (pattern_list[index.second].num_schedules >= pattern_count_limit) &&
                  (pattern_list[index.second].total_exec_time >= pattern_time_limit);
  if (decision){
    pattern_list[index.second].steady_state = true;
  } else{
    pattern_list[index.second].steady_state = false;
  }
}
bool should_schedule(std::pair<bool,int> index){
  auto& pattern_list = index.first == true ? active_patterns : steady_state_patterns;
  return !pattern_list[index.second].steady_state;
}
void update(std::pair<bool,int> index, volatile double exec_time, double unit_count){
  auto& pattern_list = index.first == true ? active_patterns : steady_state_patterns;
  if (!pattern_list[index.second].steady_state){
    pattern_list[index.second].num_schedules++;
    pattern_list[index.second].num_scheduled_units += unit_count;
    pattern_list[index.second].total_exec_time += exec_time;
//    pattern_list[index].save_exec_times.push_back((double)exec_time);	// only temporary
    ...
    size_t n1 = pattern_list[index.second].num_schedules-1;
    size_t n = pattern_list[index.second].num_schedules;
    double x = exec_time;
    double delta = x - pattern_list[index.second].M1;
    double delta_n = delta / n;
    double delta_n2 = delta_n*delta_n;
    double term1 = delta*delta_n*n1;
    pattern_list[index.second].M1 += delta_n;
...
//    this->M3 = this->M3 + term1*delta_n*(n-2)-3*delta_n*this->M2;
    pattern_list[index.second].M2 += term1;
    error_test(index);
...
//    pattern_list[index].save_jb.push_back(get_jacque_barra());
    ...
    pattern_list[index.second].num_non_schedules++;
    pattern_list[index.second].num_non_scheduled_units += unit_count;
  }
}
