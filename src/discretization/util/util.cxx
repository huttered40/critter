...
void open_symbol(const char* symbol, double curtime){
  .. so here is where we check on the specific parameterization of the kernel. I guess for now its fixed on the kernel string and its integer position in the kernel stack.
  if (symbol_timers.find(std::make_pair(symbol,symbol_stack.size()) == symbol_timers.end()){
