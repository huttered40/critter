...
bool path::initiate_comp(size_t id, volatile double curtime, double flop_count, int param1, int param2, int param3, int param4, int param5){
  ...
  if (autotuning_mode>0){
    comp_pattern_key_param1 p_id_1(-1,id,flop_count,param1,param2,param3,param4,param5);// '-1' argument is arbitrary, does not influence overloaded operators
    if (pattern_param==1){
...
void path::complete_comp(size_t id, double flop_count, int param1, int param2, int param3, int param4, int param5){
  ...
  if (autotuning_mode>0){
    comp_pattern_key_param1 p_id_1(active_patterns.size(),id,flop_count,param1,param2,param3,param4,param5);// 'active_patterns.size()' argument is arbitrary, does not influence overloaded operators
    if (pattern_param == 1){
      if (comp_pattern_map.find(p_id_1) == comp_pattern_map.end()){
...
bool path::initiate_comm(blocking& tracker, volatile double curtime, int64_t nelem, MPI_Datatype t, MPI_Comm comm,
  ...
  if (autotuning_mode>0){
    comm_pattern_key_param1 p_id_1(-1,tracker.tag,communicator_map[tracker.comm].first,communicator_map[tracker.comm].second,tracker.nbytes,(tracker.partner1 == -1 ? -1 : abs(rank - tracker.partner1)));
    if (pattern_param==1){
...
void path::complete_comm(blocking& tracker, int recv_source){
  ...
  if (autotuning_mode>0){
    ...
    comm_pattern_key_param1 p_id_1(active_patterns.size(),tracker.tag,communicator_map[tracker.comm].first,communicator_map[tracker.comm].second,tracker.nbytes,(tracker.partner1 == -1 ? -1 : abs(rank - tracker.partner1)));
    if (pattern_param == 1){
      if (comm_pattern_map.find(p_id_1) == comm_pattern_map.end()){
...
void path::propagate_patterns(blocking& tracker, int rank){
  ...
  for (auto i=0; i<active_comm_pattern_keys.size(); i++){
    comm_pattern_key_param1 id(active_comm_pattern_keys[i].pattern_index,active_comm_pattern_keys[i].tag,active_comm_pattern_keys[i].comm_size,
  ...
  for (auto i=0; i<active_comp_pattern_keys.size(); i++){
    comp_pattern_key_param1 id(active_comp_pattern_keys[i].pattern_index,active_comp_pattern_keys[i].tag,active_comp_pattern_keys[i].flops,
