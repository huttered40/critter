...
void start(bool schedule_kernels_override, bool force_steady_statistical_data_overide){
  ...
  internal::wall_timer.push_back((float)internal::computation_timer);
}

void stop(){
  volatile float last_time = MPI_Wtime();
...
void record(int variantID, int print_mode, float overhead_time){
...
int isend(const void* buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request* request){
  ...
  if (mode && track_p2p){
    ...
    if (schedule_decision){
      volatile float itime = MPI_Wtime();
