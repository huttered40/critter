...
std::string get_measure_title(size_t idx){
  ...
  measure_titles[4] = "synch time";
  measure_titles[5] = "comp time";
  measure_titles[6] = "exec time";
...
void print_cost_model_tracker_header(){
  ...
  std::cout << std::left << std::setw(text_width) << "SynchTime";
}
...
void print_cost_model_header(){
  ...
  std::cout << std::left << std::setw(text_width) << "IdleTime";
  std::cout << std::left << std::setw(text_width) << "CommTime";
  std::cout << std::left << std::setw(text_width) << "SynchTime";
  std::cout << std::left << std::setw(text_width) << "CompTime";
  std::cout << std::left << std::setw(text_width) << "CompKernelTime";
...
void print_header(size_t num_inputs, std::map<std::string,std::vector<float>>& save_info){
  ...
  stream << "\tCommCost\tSynchCost\tCompCost\tCommunicationTime\tSynchronizationTime\tComputationTime\tComputationalKernelTime\tExecTime";// critical path
  stream << "\tCommCost\tSynchCost\tCompCost\tIdleTime\tCommunicationTime\tSynchronizationTime\tComputationTime\tComputationalKernelTime\tExecTime";// per-process
  stream << "\tCommCost\tSynchCost\tCompCost\tIdleTime\tCommunicationTime\tSynchronizationTime\tComputationTime\tComputationalKernelTime\tExecTime";// vol
...
void record::print(int variantID, float overhead_time){
  ...
  if (temp_mode){
    if (is_world_root){
      ...
      for (size_t i=0; i<num_cp_measures+1; i++){//+1 for idle time (which is not present in 'num_cp_measures'
        if (i==3) std::cout << std::left << std::setw(text_width) << "0";
        else if (i>3) std::cout << std::left << std::setw(text_width) << cp_costs[i-1];
        else std::cout << std::left << std::setw(text_width) << cp_costs[i];
      }
      std::cout << "\n\n";
      ...
      for (auto i=0; i<path_index.size(); i++){
        if (path_index[i]==0){
          ...
          std::cout << std::left << std::setw(decomp_text_width+1) << "CompTime:";
        } else if (path_index[i]==5){
        ...
        std::cout << std::left << std::setw(text_width) << "SynchTime";
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "CompKernelTime";
        std::cout << std::left << std::setw(text_width) << "path";
        std::cout << std::left << std::setw(text_width) << cp_costs[cp_costs_size-path_count+i];
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "CompTime";
        std::cout << std::left << std::setw(text_width) << "path";
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << cp_costs[cp_costs_size-2*path_count+i];
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "IdleTime";
        std::cout << std::left << std::setw(text_width) << "path";
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << (path_index[i]==5 ? 0 : cp_costs[cp_costs_size-3*path_count+i]);
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "CompCost";
        std::cout << std::left << std::setw(text_width) << "path";
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << cp_costs[cp_costs_size-4*path_count+i];
        save_info.clear();
        ...
        std::cout << std::left << std::setw(text_width) << max_pp_costs[num_pp_measures+(i+1)*(num_decomp_pp_measures*list_size+4)-1];
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "CompTime";
        std::cout << std::left << std::setw(text_width) << "per-process";
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << max_pp_costs[num_pp_measures+(i+1)*(num_decomp_pp_measures*list_size+4)-2];
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "IdleTime";
        std::cout << std::left << std::setw(text_width) << "per-process";
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << max_pp_costs[num_pp_measures+(i+1)*(num_decomp_pp_measures*list_size+4)-3];
        std::cout << "\n";
        std::cout << std::left << std::setw(decomp_text_width+1) << "CompCost";
        std::cout << std::left << std::setw(text_width) << "per-process";
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << 0.0;
        std::cout << std::left << std::setw(text_width) << max_pp_costs[num_pp_measures+(i+1)*(num_decomp_pp_measures*list_size+4)-4];
        save_info.clear();
        for (int j=0; j<list_size; j++){
        ...
        for (auto& it : save_info){
          ...
          std::cout << std::left << std::setw(text_width) << 0.0;
          std::cout << std::left << std::setw(text_width) << 0.0;
          for (size_t j=0; j<num_decomp_pp_measures; j++){
            std::cout << std::left << std::setw(text_width) << it.second[j];
          }
        }
        std::cout << "\n\n";
    ...
    if (is_world_root){
      for (auto z=0; z<path_count; z++){
        ...
        for (int i=path_measure_index.size()-1; i>=0; i--){
          ...
          if (path_measure_index[i]<6){ cp_ref = cp_costs[path_measure_index[i]]; }
          else if (path_measure_index[i]==6) { cp_ref = cp_costs[7]; }
          if (path_measure_index[i]<3){ pp_ref = max_pp_costs[path_measure_index[i]]; vol_ref = vol_costs[path_measure_index[i]]; }
          else if (path_measure_index[i]==3) { pp_ref = max_pp_costs[4]; vol_ref = vol_costs[4]; }
          else if (path_measure_index[i]==4) { pp_ref = max_pp_costs[5]; vol_ref = vol_costs[5]; }
          else if (path_measure_index[i]==5) { pp_ref = max_pp_costs[6]; vol_ref = vol_costs[6]; }
          else if (path_measure_index[i]==6) { pp_ref = max_pp_costs[8]; vol_ref = vol_costs[8]; }
