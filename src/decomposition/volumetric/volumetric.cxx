...
void volumetric::collect(MPI_Comm cm){
  ...
  if (path_decomposition == 1 && path_count>0){
    ...
    for (size_t i=0; i<path_index.size(); i++){
      size_t z;
      if (path_index[i]<3){ z = path_index[i]; }
      else if (path_index[i]==3) { z = 4; }
      else if (path_index[i]==4) { z = 6; }
      else { z = 8; }
      if (rank == buffer[z].second){
        for (size_t j=0; j<num_decomp_pp_measures*list_size; j++){
          max_pp_costs[num_pp_measures+save*(num_decomp_pp_measures*list_size+4)+j] = vol_costs[num_vol_measures+j];
        }
        max_pp_costs[num_pp_measures+(save+1)*(num_decomp_pp_measures*list_size+4)-4] = vol_costs[num_vol_measures-7];
        max_pp_costs[num_pp_measures+(save+1)*(num_decomp_pp_measures*list_size+4)-3] = vol_costs[num_vol_measures-6];
        max_pp_costs[num_pp_measures+(save+1)*(num_decomp_pp_measures*list_size+4)-2] = vol_costs[num_vol_measures-3];
        max_pp_costs[num_pp_measures+(save+1)*(num_decomp_pp_measures*list_size+4)-1] = vol_costs[num_vol_measures-2];
      }
      else{
        for (size_t j=0; j<num_decomp_pp_measures*list_size+4; j++){
          max_pp_costs[num_pp_measures+save*(num_decomp_pp_measures*list_size+4)+j] = 0.;
        }
      }
      PMPI_Allreduce(MPI_IN_PLACE, &max_pp_costs[num_pp_measures+save*(num_decomp_pp_measures*list_size+4)], num_decomp_pp_measures*list_size+4, MPI_FLOAT, MPI_MAX, cm);
      save++;
    }
  }
