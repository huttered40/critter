...
int main(int argc, char ** argv){
  ...
  srand48(rank);
  double* sendbuf = (double*)malloc(msg_size*sizeof(double));
  double* recvbuf = (double*)malloc(msg_size*sizeof(double));
  ...
  for (auto i=0; i<num_iter; i++){
    critter::start();
    if (id/2==0) MPI_Reduce(sendbuf, recvbuf, msg_size, MPI_DOUBLE, MPI_SUM, root, sub_comm);
    else{
      if (color==0) MPI_Reduce(sendbuf, recvbuf, msg_size, MPI_DOUBLE, MPI_SUM, root, sub_comm);
    }
    critter::stop();
  }
