...
int main(int argc, char ** argv){
  ...
  srand48(rank);
  double* sendbuf = (double*)malloc(msg_size*sizeof(double));	// big enough for root process to scatter
  double* recvbuf = (double*)malloc(msg_size*sizeof(double));	// big enough for root process to scatter
  ...
  for (auto i=0; i<num_iter; i++){
    critter::start();
    if (id/2==0) MPI_Scatter(sendbuf, msg_size/sub_comm_size_factor, MPI_DOUBLE, recvbuf, msg_size/sub_comm_size_factor, MPI_DOUBLE, 0, sub_comm);
    else{
      if (color==0) MPI_Scatter(sendbuf, msg_size/sub_comm_size_factor, MPI_DOUBLE, recvbuf, msg_size/sub_comm_size_factor, MPI_DOUBLE, 0, sub_comm);
    }
    critter::stop();
  }
